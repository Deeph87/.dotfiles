...
FocusFirstWindow() {
    ; Obtient la liste des fenêtres visibles
    windows := WinGetList(,, "Program Manager")  ; Exclut le bureau
    
    for hwnd in windows {
        ; Vérifie si la fenêtre est visible et pas minimisée
        if (WinExist("ahk_id " hwnd) && !WinGetMinMax("ahk_id " hwnd)) {
            try {
...
; A_MenuMaskKey := "F24"  ; Touche non-assignée (vide)
